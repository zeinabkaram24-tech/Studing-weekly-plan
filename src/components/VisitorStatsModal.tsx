...
type FilterTab = 'all' | 'students' | 'visitors' | '2A' | '2B' | '2C';
...
export const VisitorStatsModal: React.FC<VisitorStatsModalProps> = ({
  ...
  const todayVisits = liveStats?.todayVisits ?? visitors.filter((v) => v.lastSeenAt >= Date.now() - 24 * 60 * 60 * 1000).length;
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-5 overflow-y-auto animate-fadeIn text-right">
      <div className="bg-white rounded-3xl w-full max-w-5xl shadow-2xl border border-slate-100 flex flex-col max-h-[92vh] overflow-hidden text-slate-800 my-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 p-4 sm:p-5 bg-slate-50 border-b border-slate-200">
          {/* 1. Students Named */}
          <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-indigo-100 shadow-2xs flex items-center justify-between">
            <div>
              <span className="text-[11px] sm:text-xs font-bold text-slate-500 block mb-1">
                سجلوا بالاسم (الطلاب)
              </span>
              <div className="flex items-baseline gap-1.5">
                ...
                <span className="text-[10px] sm:text-xs font-bold text-indigo-500">طالب مسجل</span>
              </div>
            </div>
          ...
          {/* 2. Guest Visitors */}
          <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-amber-100 shadow-2xs flex items-center justify-between">
            <div>
              <span className="text-[11px] sm:text-xs font-bold text-slate-500 block mb-1">
                الزوار (تصفح كزائر)
              </span>
              <div className="flex items-baseline gap-1.5">
                ...
                <span className="text-[10px] sm:text-xs font-bold text-amber-600">زائر</span>
              </div>
            </div>
          ...
          {/* 3. Total Usage / Visits */}
          <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-purple-100 shadow-2xs flex items-center justify-between">
            <div>
              <span className="text-[11px] sm:text-xs font-bold text-slate-500 block mb-1">
                مرات فتح واستخدام الخطة
              </span>
              <div className="flex items-baseline gap-1.5">
                <span className="text-2xl sm:text-3xl font-black text-purple-700 font-sans">
                  {totalVisits}
                </span>
                <span className="text-[10px] sm:text-xs font-bold text-purple-500">زيارة وجلسة</span>
              </div>
            </div>
          ...
          {/* 4. Active Today (24h) */}
          <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-emerald-100 shadow-2xs flex items-center justify-between">
            <div>
              <span className="text-[11px] sm:text-xs font-bold text-slate-500 block mb-1">
                النشاط خلال 24 ساعة
              </span>
              <div className="flex items-baseline gap-1.5">
                ...
                <span className="text-[10px] sm:text-xs font-bold text-emerald-600">نشط اليوم</span>
              </div>
            </div>
                    ...
                    return (
                      <tr
                        ...
                        className={`hover:bg-indigo-50/40 transition-colors ${
                          isAdmin ? 'bg-amber-50/40 font-bold' : ''
                        }`}
                      >
                        ...
                        <td className="p-3 text-center">
                          <span className="inline-block px-2.5 py-0.5 rounded-full bg-slate-100 font-mono font-bold text-indigo-700 text-xs">
                            {vis.visitCount || 1}
                          </span>
                        </td>
