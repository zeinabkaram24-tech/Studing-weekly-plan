...
export const UploadPlanFilesModal: React.FC<UploadPlanFilesModalProps> = ({
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    await Promise.all(Array.from(files).map(async (file, index) => {
      ...
      try {
        const extractedText = await extractWeeklyPlanText(file);
        const parsedTasks = parseWeeklyPlanTextWithLinks(
          extractedText,
          'sunday',
          subjects,
        ...
        console.warn('Weekly plan extraction failed; using a visible fallback task.', error);
