  ...
  return (
    ...
    >
      ...
      <div className={`flex items-start sm:items-center flex-1 min-w-0 ${compact ? 'gap-2.5 sm:gap-3' : 'gap-3.5 sm:gap-4'}`}>
        <div
          className={`${compact ? 'w-9 h-9 sm:w-10 sm:h-10 rounded-xl' : 'w-11 h-11 sm:w-13 sm:h-13 rounded-2xl'} flex items-center justify-center shrink-0 border ${iconBg} ${iconColor} ${borderColor} shadow-2xs group-hover:scale-105 transition-transform`}
        >
          <SubjectIcon name={subject?.iconName || 'BookOpen'} className={`${compact ? 'w-4 h-4 sm:w-4.5 sm:h-4.5' : 'w-5 h-5 sm:w-6 sm:h-6'} stroke-[2]`} />
        </div>
