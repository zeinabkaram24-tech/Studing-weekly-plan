...
// Refresh the official ICT and Arabic tasks in a previously saved Week 1
// archive without overwriting other subjects or later administrator plans.
function mergeOfficialWeek1Tasks(entry: WeeklyPlanArchiveEntry): WeeklyPlanArchiveEntry {
  ...
  const tasksBySection = (['2A', '2B', '2C'] as GradeSection[]).reduce(
    (result, section) => {
      const saved = entry.tasksBySection?.[section] || [];
      const keepOtherSubjects = saved.filter(
        (task) => task.subjectId !== 'ict' && task.subjectId !== 'arabic'
      );
      const officialTasks = defaultsBySection[section].filter(
        (task) => task.subjectId === 'ict' || task.subjectId === 'arabic'
      );
      result[section] = [...keepOtherSubjects, ...officialTasks];
      return result;
    },
